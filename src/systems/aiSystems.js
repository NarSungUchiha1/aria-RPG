...
// ── Owner recognition — single source of truth in utils/identity ─────────────
// (re-exported below for the modules that import isOwner from here)
const { digitsOnly, isOwner, isAdminId } = require('../utils/identity');
...
async function _handleAriaCommandInner(sock, jid, msg, userId, question, { isAdmin = false, blockedSet = null } = {}) {
    ...
    // Master is the OWNER ALONE. Group admins are not Vortex: they must never
    // get the Master persona, the Eva true name, or adminAI's database access.
    // (`isAdmin` only lifts the rate limit — see isPrivileged below.)
    const isMaster     = owner;
    const isPrivileged = owner || isAdmin; // queue/cooldown skip only

    // ── ONLY the owner gets adminAI (full SQL over the game DB) ───────────────
    if (isMaster && question?.trim()) {
