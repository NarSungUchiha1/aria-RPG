module.exports = {
    Tank: [
        { name: "Strike",        type: "damage", stat: "stamina",  multiplier: 1.2,                           cooldown: 1,   cost: 0  },
        { name: "Shield Bash",   type: "damage", stat: "stamina",  multiplier: 1.6,                           cooldown: 2,   cost: 5  },
        { name: "Fortify",       type: "buff",   effect: "defense", value: 25, duration: 60,                  cooldown: 120, cost: 8  },
        { name: "Taunt",         type: "debuff", effect: "attack",  value: -15, duration: 3,                  cooldown: 30,  cost: 6  },
        { name: "Iron Wall",     type: "shield", value: 150,                                                   cooldown: 60,  cost: 12 },
        { name: "Earth Shatter", type: "damage", stat: "strength", multiplier: 1.8, stun: true,               cooldown: 4,   cost: 10 }
    ],
    Assassin: [
        { name: "Strike",        type: "damage", stat: "strength", multiplier: 1.0,                           cooldown: 1,   cost: 0  },
        { name: "Backstab",      type: "damage", stat: "agility",  multiplier: 1.8,                           cooldown: 2,   cost: 6  },
        { name: "Shadow Step",   type: "buff",   effect: "agility", value: 30, duration: 2,                   cooldown: 45,  cost: 8  },
        { name: "Poison Dagger", type: "debuff", effect: "strength", value: -8, duration: 3,                  cooldown: 3,   cost: 7  },
        { name: "Fatal Strike",  type: "damage", stat: "agility",  multiplier: 2.0,                           cooldown: 5,   cost: 12 },
        { name: "Smoke Bomb",    type: "debuff", effect: "agility", value: -15, duration: 2,                  cooldown: 45,  cost: 5  }
    ],
    Mage: [
        { name: "Strike",           type: "damage", stat: "strength",     multiplier: 1.0,                    cooldown: 1,   cost: 0  },
        { name: "Fireball",         type: "damage", stat: "intelligence", multiplier: 1.6,                    cooldown: 1,   cost: 8  },
        { name: "Arcane Blast",     type: "damage", stat: "intelligence", multiplier: 1.4, aoe: true,         cooldown: 3,   cost: 12 },
        { name: "Mana Shield",      type: "shield", value: 40, duration: 3,                                   cooldown: 5,   cost: 10 },
        { name: "Frost Nova",       type: "damage", stat: "intelligence", multiplier: 1.2, freeze: true,      cooldown: 4,   cost: 9  },
        { name: "Arcane Intellect", type: "buff",   effect: "intelligence", value: 25, duration: 3,           cooldown: 45,  cost: 12 }
    ],
    Healer: [
        { name: "Strike",            type: "damage",  stat: "strength",     multiplier: 1.0,                  cooldown: 1,   cost: 0  },
        // Basic recovery — reliable but modest output
        { name: "Heal",              type: "heal",    stat: "intelligence", multiplier: 3.0, baseHeal: 20,    cooldown: 20,  cost: 8  },
        { name: "Blessing",          type: "buff",    effect: "stamina",    value: 15, duration: 3,           cooldown: 45,  cost: 12 },
        { name: "Cleanse",           type: "cleanse",                                                          cooldown: 3,   cost: 5  },
        // Burst heal — nearly 3x Heal's output, also removes one debuff
        { name: "Holy Light",        type: "heal",    stat: "intelligence", multiplier: 8.0, baseHeal: 80,
          cleanse: true,                                                                                        cooldown: 60,  cost: 14 },
        { name: "Divine Protection", type: "shield",  value: 60, duration: 2,                                 cooldown: 45,  cost: 10 }
    ],
    Berserker: [
        { name: "Strike",      type: "damage", stat: "strength", multiplier: 1.0,                             cooldown: 1,   cost: 0  },
        { name: "Rage Slash",  type: "damage", stat: "strength", multiplier: 1.8,                             cooldown: 10,  cost: 0  },
        { name: "Bloodlust",   type: "buff",   effect: "strength", value: 30, duration: 3,                    cooldown: 45,  cost: 10 },
        { name: "Smash",       type: "damage", stat: "strength", multiplier: 2.0, stun: true,                 cooldown: 10,  cost: 12 },
        { name: "Frenzy",      type: "damage", stat: "strength", multiplier: 1.4, hits: 3,                    cooldown: 10,  cost: 15 },
        { name: "Intimidate",  type: "debuff", effect: "defense", value: -15, duration: 2,                    cooldown: 45,  cost: 8  }
    ]
};